 ...
 */
{
    ...
    my_image.src = 'https://images.pexels.com/photos/207171/pexels-photo-207171.jpeg';
    ...
    class Grid {
        ...
        toggleProject(action) {
            ...
            this.showRevealer().then(() => {
                this.DOM.fullviewItems[this.current].style.display = this.isGridHidden ? 'grid' : 'none';
                // this.DOM.fullviewItems[this.current].style.opacity = this.isGridHidden ? 1 : 0;
                this.DOM.fullview.style.display = this.isGridHidden ? 'grid' : 'none';
        ...
        toggleRevealer(action, dir) {
            return new Promise((resolve, reject) => {
                ...
                if ( action === 'show' ) {
                    this.DOM.revealer.style.backgroundImage = this.movable[this.current].DOM.el.dataset.revealerColor;
                ...
                TweenMax.to( this.DOM.revealer, action === 'show' ? 1 : 1, {
                    ease: 'Expo.easeInOut',
                    // ease: action === 'show' ? 'Expo.easeInOut' : 'Quint.easeOut',
                    y: action === 'show' ? '-80%' : (dir === 'up' ? '-200%' : '0%'),
                    onComplete: resolve
                });
            });
        }
